...
def main():
    ...
    if background_image_file and dartboard_image_file:
        ...
        background_image_cropped = crop_image(background_image, x1, y1, x2, y2)
        background_image_preprocess = preprocess(background_image_cropped)

        _, score_contour = extracting_roi(background_image_preprocess)
        
        foreground_image_cropped = crop_image(dartboard_image, x1, y1, x2, y2)

        forground_image_preprocess = preprocess(foreground_image_cropped)

        diff = Difference(background_image_preprocess, forground_image_preprocess)
        diff = Remove_noise(diff)
        # st.write("diff dartboard:", diff)
        # st.image(forground_image, caption='fore dartboard')


        result_img, diff_thresh, locations = Get_locations(diff, forground_image_preprocess, score_contour)
        ...
        # Calculate total dart score
        total_dart_score = sum(dart_scores)

        # Display dart scores (outside the loop)
        st.write("Dart Score:", dart_scores)
